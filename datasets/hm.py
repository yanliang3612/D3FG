...
from .parsers import parse_protein_ligand_pairs_with_hm
import logging
import joblib
import lmdb
import pickle

@register_dataset('hotmapelab')
def get_hotmapelaborate_dataset(cfg, transform):
    # split_path_test = torch.load(cfg.split_path)['test']
    return PairedHMElab(
        raw_path = cfg.raw_path,
        data_path = cfg.get('data_path', None),
        hotspots_path = cfg.get('hotspots_path', None),
        preserve_pair = [()],
        transform = transform,
    )

class PairedHMElab(Dataset):

    MAP_SIZE = 16*(1024*1024*1024) 
    def __init__(self, raw_path, hotspots_path, data_path=None, preserve_pair=(), transform=None) -> None:
        super().__init__()
        ...
        self.name2id_path = os.path.join(os.path.dirname(self.data_path), 'hotmapelab_name2id.pt')
        self.processed_paired_path = os.path.join(os.path.dirname(self.data_path), 'hotmapelab_processed.lmdb')
        self.db = None
    ...
    def _preprocess_paired(self):
        ...
        for i, (pocket_fn, ligand_fn, _, rmsd_str) in enumerate(process_bar):
            try:
                ...
                hs_path = os.path.join(self.hotspots_path, pocket_fn[:-4] +'_hotspots.csv')
                ...
                if not os.path.exists(hs_path):
                    logging.warning(f"Hotspots CSV not found: {hs_path}")
                    continue
        ...
        # [parse_protein_ligand_pairs_with_hm(task)
        ...
        data_list = joblib.Parallel(
            ...
            joblib.delayed(parse_protein_ligand_pairs_with_hm)(task)
            for task in tqdm(tasks, dynamic_ncols=True, desc='Preprocessing paired protein ligand features')
        )
