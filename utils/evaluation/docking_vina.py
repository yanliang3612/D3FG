...
class VinaDock(object): 
    def __init__(self, lig_pdbqt, prot_pdbqt, pose_pdbqt): 
        self.lig_pdbqt = lig_pdbqt
        self.prot_pdbqt = prot_pdbqt
        self.pose_pdbqt = pose_pdbqt
    ...
    def dock(self, score_func='vina', seed=0, mode='dock', exhaustiveness=8, save_pose=False, **kwargs):  # seed=0 mean random seed
        ...
        if mode == 'score_only': 
            ...
            if save_pose:
                v.write_pose(self.pose_pdbqt, overwrite=True)
        else:
            raise ValueError
...
class GninaDockingTask(BaseDockingTask):
    @classmethod
    def from_generated_mol(cls, ligand_path, protein_path, **kwargs):
        return cls(protein_path, ligand_path, **kwargs)
    
    def __init__(self, protein_path, ligand_path, tmp_dir='./tmp', center=None,
                 size_factor=1., buffer=5.0):
        
        super().__init__(protein_path, ligand_path)
        ...
        self.ligand_path_tmp = os.path.join(self.tmp_dir, self.ligand_id + '.sdf')

        self.recon_ligand_mol = ligand_rdmol
        ligand_rdmol = Chem.AddHs(ligand_rdmol, addCoords=True)

        sdf_writer = Chem.SDWriter(self.ligand_path_tmp)
    ...
    def run(self, save_pose=False):
        ligand_dock_out = self.ligand_path[:-4] + '_gninadocked' + '.sdf.gz'
        commands = """
        conda activate {env}
        /root/gnina --minimize -r {receptor_file} -l {ligand_file} --autobox_ligand {ligand_ref_file} 
        """.format(
            env = 'torch1.8', 
            receptor_file=self.receptor_path, 
            ligand_file=self.ligand_path_tmp,
            ligand_ref_file=self.ligand_path_tmp
            )
        if save_pose:
            commands = """
            conda activate {env}
            /root/gnina --minimize -r {receptor_file} -l {ligand_file} --autobox_ligand {ligand_ref_file} -o {out_ligand_file}
            """.format(
                env = 'torch1.8', 
                receptor_file=self.receptor_path, 
                ligand_file=self.ligand_path_tmp,
                ligand_ref_file=self.ligand_path_tmp,
                out_ligand_file = ligand_dock_out
                )
        ...
        out_info = proc.stdout.readlines()
        cnn_affinity = float(out_info[-2].split()[1])
        cnn_score = float(out_info[-3].split()[1])

        return{'cnn_affinity': cnn_affinity, 'cnn_score': cnn_score}
...
class VinaDockingTask(BaseDockingTask):
    ...
    def run(self, mode='dock', exhaustiveness=8, save_pose=False, **kwargs):
        ligand_pdbqt = self.ligand_path_tmp[:-4] + '.pdbqt'
        protein_pqr = self.receptor_path[:-4] + '.pqr'
        protein_pdbqt = self.receptor_path[:-4] + '.pdbqt'
        ligand_dock_pdbqt = self.ligand_path[:-4] + '_vinadocked' + '.pdbqt'

        lig = PrepLig(self.ligand_path_tmp, 'sdf')
        ...
        dock = VinaDock(ligand_pdbqt, protein_pdbqt, pose_pdbqt=ligand_dock_pdbqt)
        dock.pocket_center, dock.box_size = self.center, [self.size_x, self.size_y, self.size_z]
        score, pose = dock.dock(score_func='vina', mode=mode, exhaustiveness=exhaustiveness, save_pose=save_pose, **kwargs)
