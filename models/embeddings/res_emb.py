...
from ..utils.geometry import construct_3d_basis_include_single_atom, global_to_local, get_backbone_dihedral_angles
from ..modules.layers import AngularEncoding
from ..utils.constants import BBHeavyAtom, AA
...
class ResidueEmbedding(nn.Module):
    ...
    def forward(self, aa, res_nb, chain_nb, pos_atoms, mask_atoms, fragment_type, structure_mask=None, type_mask=None):
        """
        Args:
            aa:         (N, L).
            res_nb:     (N, L).
            ...
            type_mask:  (N, L), mask out unknown amino acids to generate.
        """
        N, L = aa.size()
        ...
        if type_mask is not None:
            ...
            aa = torch.where(type_mask, aa, torch.full_like(aa, fill_value=AA.UNK))
